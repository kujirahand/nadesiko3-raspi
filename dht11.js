 ...
 */
function read_dht11(pin) {
  // init data
  var vals= Buffer.alloc(5);
	vals.fill(0);
	...
	if (data.length < 42 || data.length > 43) {
		...
		return {'温度': 99, '湿度': 99, 'エラー':'サイズの異常:' + data.length};
	}
	...
	if (vals[0] < 20 || vals[0] > 90) {
		...
		return {'温度':vals[2], '湿度':vals[0], 'エラー':'湿度異常'};
	}

	/* Temperature range is 0 - 50C */
	if (vals[2] > 50) {
		dbg("Bad temperature: %d", vals[0]);
		return {'温度':vals[2], '湿度':vals[0], 'エラー':'温度異常'};
	}
  ...
  var tmp = parseFloat(`${vals[2]}.${vals[3]}`)
  var hum = parseFloat(`${vals[0]}.${vals[1]}`)
	return {'温度':tmp, '湿度':hum, 'エラー':''};
}


module.exports = read_dht11
